...
async def get_live_meteorological_data(lat: float, lon: float) -> Dict[str, Any]:
 ...
 try:
 weather_url = fhttps://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&units=metric&appid={OPENWEATHER_API_KEY}
 forecast_url = fhttps://api.openweathermap.org/data/2.5/forecast?lat={lat}&lon={lon}&units=metric&appid={OPENWEATHER_API_KEY}
 pollution_url = fhttps://api.openweathermap.org/data/2.5/air_pollution?lat={lat}&lon={lon}&appid={OPENWEATHER_API_KEY}
 ...
 city_name = data.get(name, Hathras)
...
async def _get_open_meteo_fallback(lat: float, lon: float) -> Dict[str, Any]:
 ...
 url = fhttps://api.open-meteo.com/v1/forecast?latitude={lat}&longitude={lon}&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone=auto
